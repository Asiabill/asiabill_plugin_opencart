<?php 
class ModelExtensionPaymentAsiabillIdeal extends Model {

  	public function getMethod($address) {

  		$helper = new Asiabill\Helper();
  		$code = $helper->payMethodCode('ideal');
        ...
        $logo = '';
      	...
      	if( $this->config->get('payment_asiabill_general_logo_type') == '1' ){
            $logo = '<img src="catalog/view/theme/default/image/asiabill/ideal.png">';
		}

		if ($status) {
      		$method_data = array(
        		'code'       => $code,
        		'title'      => $this->config->get('payment_'.$code.'_title').$logo,
